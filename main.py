...
def new_round():
    ...
    number = random.randint(10, 100)  # Random number for square root calculation
    ...
    number_line_start_x = int(width * 0.25)  # Start of the number line
    number_line_end_x = int(width * 0.75)  # End of the number line
...
def draw_number_line():
    ...
    number_line_start_x = int(width * 0.25)  # Ekranın %25'inden başlat
    number_line_end_x = int(width * 0.75)    # Ekranın %75'ine kadar uzat
...
while running:
    ...
    number_text = font.render(f"√({number}) = ?", True, PHOSPHOR_YELLOW)

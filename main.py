...
pygame.display.set_caption("Karekökmetre")
...
def new_round():
    global number, sqrt_number, start_time, user_input, arrow_position, target_position, shooting, question,i
    
    try:
        question = port.readline().decode().strip()  # Bytes'tan string'e çevir ve boşlukları temizle
        sqrt_number = math.sqrt(int(question))  # Sayıya çevir
    except (ValueError, UnicodeDecodeError):
        sqrt_number = random.randint(1, 100)  # Hata olursa rastgele bir sayı seç
    ...
    if i == 1 or i == 4:
        pygame.mixer.music.load("41.mp3")  
        pygame.mixer.music.play()
    elif i == 2:
        pygame.mixer.music.load("65.mp3")  
        pygame.mixer.music.play()
    elif i == 3:
        pygame.mixer.music.load("31.mp3")  
        pygame.mixer.music.play()
...
while running:
    ...
    if shooting == True:
      ...
      # Eğer ok henüz atılmadıysa düz çizim
    ...
    for event in pygame.event.get():
        ...
        # Klavyeden girilen sayıları yakala
            ...
            if event.key == pygame.K_RETURN:
                try:
                    if user_input.split("-")[0].isdigit() and user_input.split("-")[1].isdigit() and int(user_input.split("-")[0]) - sqrt_number > -1 and int(user_input.split("-")[1]) - sqrt_number < 1 and int(user_input.split("-")[0]) - sqrt_number < 0 and int(user_input.split("-")[1]) - sqrt_number > 0:

                        target_position = [int(user_input.split("-")[0]) * (width // 20), height - 50]
                        shooting = True 
                        score += 1  
